...
- `https://satakuntaevents.fi/api/v2/`

Havainto:

- Julkinen, dokumentoitu API
- JSON-LD-pohjainen tapahtumarajapinta
- Tukee suodattimia kuten:
  - `text`
  - `start`
  - `end`
  - `location`
...
- Erittain lupaava
- Ei kannata scrapata yksittaisia aluekalenterisivuja, jos taustalla on jo tama API

### 3. Keski-Suomi Events

Lahde:

- `https://keskisuomievents.fi/apispec/index.html`

Havainto:

- Julkinen Linked Events -tyylinen API
- Tukee tapahtumien hakua ajalla, tekstilla, paikalla ja avainsanoilla

Arvio:

- Erittain lupaava
- Todennakoisesti helppo integroida samalla mallilla kuin muut API-lahteet

### 4. Aluekalenteri

Lahde:

- `https://aluekalenteri.fi/`

Havainto:

- Aluekalenteri kertoo itse tarjoavansa rajapinnan kolmansille osapuolille
- Sama alusta toimii useiden alueellisten tapahtumakalenterien taustalla
- Esimerkkeina sivu mainitsee mm. Satakunta Eventsin ja Keski-Suomi Eventsin

Arvio:

- Tarkeampi metalahde kuin yksittainen integroitava endpoint
- Hyodyllinen uusien saman perheen instanssien tunnistamiseen

### 5. Menoinfo

Lahteet:

- `https://www.menoinfo.fi/`
- `https://uusimaa.menoinfo.fi/`

Havainto:

- Laaja alueellinen tapahtumakalenteri
- Haku ei vahvistanut yhta selvasti dokumentoitua julkista API:a kuin Satakunta/Keski-Suomi -instansseissa

Arvio:

- Kiinnostava lahde
- Ei ensimmainen toteutuskohde ennen kuin varmistetaan koneellinen rajapinta

### 6. Helsinki / Espoo Linked Events

Lahteet:

- Helsinki:
  - `https://www.opendata.fi/data/en_GB/dataset/linked-events-tapahtumarajapinta`
- Espoo:
  - `https://www.avoindata.fi/data/en_GB/dataset/espoon-tapahtumarajapinta`

Havainto:

- Julkiset avoimen datan tapahtumarajapinnat
- Soveltuvat alueelliseen tapahtumahakuun
- Eivat ole pyorailyspesifeja, mutta pyorailytapahtumia voi hakea tekstilla tai avainsanoilla

Arvio:

- Hyvia lisa-alueita etenkin paakaupunkiseudulle
- Vaativat huolellisen filtteroinnin, jotta mukaan ei tule paljon eparelevantteja tapahtumia
